...
- **Build workflow** (`.github/workflows/build.yml`) runs on `release published`: builds installers (Linux `.deb`/`.rpm`/`.AppImage`, macOS `.dmg`/`.app.tar.gz`, Windows `.msi`/`.exe`) and uploads them to the release.
