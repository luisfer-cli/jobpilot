...
- **Build** runs in the same workflow (`release.yml`, job `build`), right after release-please creates a release. It uses `tauri-apps/tauri-action` to build and sign installers (Linux `.AppImage`, Windows `.msi`/`.exe`), generate `latest.json`, and upload them to the release. Manual rebuilds: `gh workflow run Release -f tagName=vX.Y.Z`.
- Gotcha: release-please creates the release with `GITHUB_TOKEN`, which does NOT trigger other workflows — that's why build and release-please live in the same workflow (the `build` job is gated by `release_created`).
